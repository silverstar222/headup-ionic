...
MapApp.config(['$stateProvider', '$urlRouterProvider', function($stateProvider, $urlRouterProvider) {
  $stateProvider
    .state('menu', {url: "", abstract: true, templateUrl: "menu.html"})
    .state('menu.home', {url: '/home', views: {'menuContent': {templateUrl: 'home.html', controller: 'GpsCtrl'} }  })
    .state('menu.map', {url: '/map', views: {'menuContent': {templateUrl: 'map.html', controller: 'GpsCtrl'} }  })
...
MapApp.controller('GpsCtrl', ['$scope','$ionicPlatform', '$location',
  function($scope, $ionicPlatform, $location) {
    ...
    $scope.gotoLocation(current_location.latlng.lat, current_location.latlng.lng);
...
MapApp.directive("appMap", function ($window) {
  return {
    ...
    link: function (scope, element, attrs) {
      ...
      function createMap() {
        console.log("map: create wmap start");
        var mapOptions = {
          zoom: 13,
